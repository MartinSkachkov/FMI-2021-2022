...
bool findWord(char grid[][GRID_SIZE], bool visited[][GRID_SIZE], char* word, size_t currX, size_t currY, size_t currWordLen, int& countLen, size_t letter) {
	//base case
...
bool crossword(char grid[][GRID_SIZE], char** words, bool visited[][GRID_SIZE], size_t n,int& maxLen) {
	...
	for (size_t word = 0; word < n; word++) {
		...
		for (size_t gridX = 0; gridX < GRID_SIZE; gridX++)
		{
			...
			{
				int countLen = 0;
				foundWord = findWord(grid, visited, words[word], gridX, gridY, currWordLen, countLen, 0);
				if (!foundWord) {
					continue;
				}
				else {
...
int main() {
	...
	cout << "crossword can be solved? " << boolalpha << crossword(grid, words, visited, n, maxLen) <<'\n' 
		//it changes the maxLen in the debugger but still prints zero somehow???
		<<"max word len: " << maxLen;
