bool validatePath(char* path) {
	...
	{
		if (path[i] != 'E' && path[i] != 'W' && path[i] != 'N' && path[i] != 'S')
		{
			return false;
		}
	}
...
bool isValidSituation(bool** matrix, int n, int startX, int startY) {
	return startX >= 0 && startY >= 0 && startX <= n-1&& startY <= n-1&& matrix[startX][startY] == 1;
}
...
bool hasPath(bool** matrix, int n, char* path) {
	...
	for (int i = strlen(path) - 1; i >= 0; i--)
	{
		...
		switch (path[i])
		{
		case 'E': startY--; break;//go west
		case 'W': startY++; break;//go east
		case 'N': startX++; break;//go south
		case 'S': startX--; break;//go north
		}
	}
